...
export default function SettingForm({ id }: { id: string }) {
  ...
  const [isNewPasswordAgainTouched, setIsNewPasswordAgainTouched] =
    useState(false);

  const [isCurrentPasswordVisible, setCurrentPasswordIsVisible] =
    useState(false);
  const [isNewPasswordVisible, setNewPasswordIsVisible] = useState(false);
  const [isNewPasswordAgainVisible, setNewPasswordAgainIsVisible] =
    useState(false);
  const toggleCurrentPasswordVisibility = () =>
    setCurrentPasswordIsVisible(!isCurrentPasswordVisible);
  const toggleNewPasswordVisibility = () =>
    setNewPasswordIsVisible(!isNewPasswordVisible);
  const toggleNewPasswordAgainVisibility = () =>
    setNewPasswordAgainIsVisible(!isNewPasswordAgainVisible);

  const isCurrentPassowrdInvalid = useMemo(() => {
    if (!isCurrentPasswordTouched) return false;
    return (
      state.errors?.currentPassword !== undefined || currentPasswordValue === ''
    );
  ...
  const isNewPasswordInvalid = useMemo(() => {
    ...
    return (
      ...
      newPasswordValue !== newPasswordAgainValue
    );
    ...
    >
      ...
      <section className='flex flex-col gap-2'>
        ...
        <p className='text-xs text-foreground-600'>
          8〜32文字で、大文字、小文字、数字、特殊文字(#?!@$%^&*-)を含む必要があります。
        </p>
        <PasswordInput
          isRequired
          required
          ...
          errorMessage={
            state.errors?.currentPassword || 'パスワードは必須です。'
          }
